...
def calculate_nearest_centroid(points, centroids):
    ...
    # for i in range(len(centroids)):
    #     if len(clusters[i]) == 0:
    #         clusters[i].append(list(centroids[i])) TODO для отсутствующих точек. 
    #         Сейчас не нужно, т.к. для таких центроид точки не печатаются, а центроиды остаются на месте
